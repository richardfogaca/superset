...
function Heatmap(element, props) {
  ...
  function ordScale(k, rangeBands, sortMethod, customOrder = '') {
    let domain = {};
    ...
    if (sortMethod === 'alpha_asc') {
      ...
      domain = customSort(keys, customOrder);
    }
  ...
  const xScale = ordScale('x', null, sortXAxis, customOrderXAxis);
  const yScale = ordScale('y', null, sortYAxis, customOrderYAxis);
  const xRbScale = ordScale('x', [0, hmWidth], sortXAxis, customOrderXAxis);
  const yRbScale = ordScale('y', [hmHeight, 0], sortYAxis, customOrderYAxis);
    ...
    .html(function () {
      ...
      if (m in matrix && n in matrix[m]) {
        ...
        if (Array.isArray(metrics)) {
          s += `<div><b>${getMetricLabel(metrics[0])}: </b>${valueFormatter(
            obj.v,
          )}<div>`;
        } else {
          s += `<div><b>${getMetricLabel(metrics)}: </b>${valueFormatter(
            obj.v,
          )}<div>`;
        }

        // When there are more than 1 metric, show using extra_metrics
        if (obj.extra_metrics) {
          Object.entries(obj.extra_metrics).forEach(
            ([metricName, metricValue]) => {
              s += `<div><b>${getMetricLabel(metricName)}: </b>${valueFormatter(
                metricValue,
              )}<div>`;
            },
          );
